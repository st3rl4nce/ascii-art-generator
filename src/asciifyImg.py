...
def asciify(inp, oup, bg, rs):
    ...
    rs: resolution of the output => 1: resizes the output to input resolution, 0: none
    ...
    out_image.save(oup)
    # resizing the image to match the size of the source
    if rs == 1:
        img = cv2.imread(oup)
        out_image = cv2.resize(img, (width, height), interpolation=cv2.INTER_CUBIC)
        cv2.imwrite(oup, out_image)
    return oup
